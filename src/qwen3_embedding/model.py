...
"""
Qwen3-Embedding backbone, BF16 (NDD Day-2 Challenge)
====================================================

Stage 1 of the vLLM Neuron model-onboarding guide: the ``llama3/model.py``
dense template copied and modified for the Qwen3 backbone that
Qwen/Qwen3-Embedding-8B is built on. Dense, GQA, QK-norm, SiLU, standard RoPE.

Supported parallelism: TP, SP, DP.
Reference model: Qwen/Qwen3-Embedding-8B
(dense, 36 layers, 32Q/8KV GQA, head_dim=128, no sliding window).

Changes made to the Llama template (identified in step0_arch_diff.py):
  - Per-head QK-norm (q_norm / k_norm on head_dim) applied BEFORE RoPE —
    Llama has no QK-norm at all. Prefill applies it in torch; decode enables
    it in the fused megakernel via ``rmsnorm_QK_pre_rope_enabled=True``.
  - Standard rotate_half RoPE with rope_theta=1e6 — Llama-3.1 uses
    piecewise "llama3" rope_scaling, which is removed here.
  - rms_norm_eps 1e-6 (Llama: 1e-5); dims per config.py.
  - Weight mappings gain q_norm/k_norm (replicated, no TP sharding).
